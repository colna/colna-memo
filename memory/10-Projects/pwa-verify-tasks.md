...
- **P0(前端可独立交付)**:`Chat/index.tsx` 加 `useEffect`——`convData.hasProbe` 时按 `probeType` 弹对应抽屉;抽屉补传 `rewardText`(centsToText(probeRewardCents))、`countdownSeconds`(probeTimeoutSeconds)、`promptText`(probePrompt,仅 Voice)、`onSent`、`onCountdownEnd`(现只传了 `peerName/onVerify`)。mock 审核下走 `/dev/photo-task`、`/dev/voice-task` 全流程自测。
