...
## 9. 验证毛玻璃/渐变这类分层视觉：截图取像素，别靠肉眼（2026-09-18 补充）

**需求**：Luka 底栏改毛玻璃渐变（多层 `BlurView` + `LinearGradient`），要确认「渐变真的
在渲染、几何盖到整条栏、方向是上浅下深」——平色页面上肉眼几乎看不出差异。

- **竖直采样一条线**：`xcrun simctl io booted screenshot x.png` 后，
  `magick x.png -format "%[pixel:p{30,Y}]" info:` 从栏顶到栏底每隔 40px 取一次，
  拿到的是实际合成色；和手算的 alpha 合成对账，几何/方向/透明度一次全清楚。
- **图层几何有疑问时用临时调试底**：给可疑层临加 `backgroundColor: "rgba(255,0,0,0.5)"`，
  截图看红色覆盖范围（红 0.5 压页面色的混合值是可精确预期的），确认后删掉。
  比重读 Yoga 布局规则快得多。
- **坑**：渐变端点写「页面同色 + 低 alpha」时，在页面底色上**数学上不可见**
  （alpha 只在混不同色时改变结果），容易误判成「没生效」。毛玻璃要看得见，浅端得用
  `Colors.surface` 这类更亮的色做雾，而不是页面色的透明版。
- 改完在 Metro 热更下秒级可见；simctl 没有触摸指令，滚动验证用第 8 节的 CGEvent 注入。
