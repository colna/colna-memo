...
## 探头角色绝对定位：父容器的上边距要让它「占位」

把「从卡片上沿探头的角色」做成绝对定位（画在卡片之后，`top: 5 − 素材高`）时，若卡片上边距只按卡片自己的视觉间距给，角色悬在卡片上方的那几十 pt 会盖住页头标题 —— **绝对定位不参与布局**，卡片不会因为它往下让。luka 退出确认页第一版 `mt-7`（28pt）就是这个毛病，截图里仓鼠耳朵压在 "Sign out" 上。

- 修法：父容器上边距 ≥ 角色在卡片上方的高度 + 页头与它之间的间隙（该屏最终用 `mt-[96px]`）。
