...
## 二次转折(2026-07-28 深夜):目标是 App 独家「视频测评」板块

用户截图确认:要爬的是京东 **App 内**「大家评」下的 **「视频测评 46」** 聚合板块(买家测评短视频)。**网页端(H5/PC)没有这个板块**——用户明确"App 内看到,网页没看到"。

**后果:浏览器扩展方案对这个板块作废**(网页无此数据)。只能走 **App 爬取**:
- 路线 A：抓包 + App native 签名逆向(比 h5st 难,新版有 SSL Pinning 需 Frida 绕)+ 服务端复现。
- 路线 B：App 自动化(Appium/云机)真机跑,抓接口响应。慢但绕签名。

**成本**:App 逆向方案 ¥3~8万+,维护重(签名随版本变),合规风险更高。远超浏览器方案(¥1~2万)。

**待定关键**:需拿到 App「视频测评/看更多测评」那条接口(URL+body+响应)才能定案——签名弱则服务端能直接吐 JSON(A),签名硬则只能 App 自动化(B)。等用户手机抓包。
